try{db.createUser({user:"mloger",pwd:"mloger_pwd",roles:[{"role":"readWrite","db":"mloger"}]})}catch(e){print(e)}
try{ db.getCollection("apis").createIndex({"status":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("apis").createIndex({"host":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("apis").createIndex({"path":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("apis").createIndex({"mzip":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("apis").createIndex({"crypto":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("apis").createIndex({"schema":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("apis").createIndex({"rd_path":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("apis").createIndex({"req_id":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("connections").createIndex({"live":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("connections").createIndex({"client_host":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("intercept").createIndex({"status":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("intercept").createIndex({"type":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("repeater").createIndex({"describe":1},{background:1, unique:false })}catch(e){print(e)}
try { db.getCollection("repeater").createIndex({_id: 1});} catch (e) {  print(e); }
try{ db.getCollection("repeater").createIndex({"host":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("repeater").createIndex({"describe":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("repeater").createIndex({"overview_repeater_id":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("request_data").createIndex({"path":-1,"host":-1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("request_data").createIndex({"path":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("request_data").createIndex({"host":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("request_data").createIndex({"update_time":1},{background:1, unique:false })}catch(e){print(e)}
try { db.getCollection("request_data").createIndex({_id: 1});} catch (e) {  print(e); }
try{ db.getCollection("request_data").createIndex({"client_ip":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("request_data").createIndex({"exclude":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("request_data").createIndex({"wait_response":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("request_data").createIndex({"intercept":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("request_data").createIndex({"user_name":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("request_data").createIndex({"isshow":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("response_data").createIndex({"update_time":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("response_data").createIndex({"req_id":1},{background:1, unique:false })}catch(e){print(e)}
try { db.getCollection("response_data").createIndex({_id: 1});} catch (e) {  print(e); }
try{ db.getCollection("response_data").createIndex({"client_ip":-1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("response_data").createIndex({"exclude":-1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("tcp").createIndex({"exclude":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("tcp").createIndex({"server_id":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("tcp").createIndex({"client_id":1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("websocket").createIndex({"conn_id":-1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("websocket").createIndex({"client_id":-1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("websocket").createIndex({"server_id":-1},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("apis").createIndex({"host":"text","path":"text"},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("repeater").createIndex({"host":"text","path":"text","content":"text"},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("request_data").createIndex({"host":"text","path":"text","content":"text"},{background:1, unique:false })}catch(e){print(e)}
try{ db.getCollection("response_data").createIndex({"content":"text"},{background:1, unique:false })}catch(e){print(e)}
