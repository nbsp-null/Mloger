...
db['connections'].update({'type': 'tcp', 'live': True}, {'$set': {'live': False, 'timestamp_end': time.time()}},
                         multi=True)
tcp_conns_dict = {}
req_id_dict = {}
...
def request(flow: http.HTTPFlow) -> None:
    flow.request.host = flow.request.headers.get('Host', flow.request.host)
    ...
    if not in_black:
        try:
            if flow.response:
                ...
                if len(flow.response.content) > 1048576:
                ...
                if intercept:
                    ...
                    if intercept_data and intercept_data['forward']:
                        if intercept_data['edited']:
                            if type(intercept_data['response_edited']) is str:
...
def tcp_message(flow):
    ...
    if not message.from_client:
        # 超长数据包等待拼接处理
        if len(message.content) > 4090:
            return
        else:
            # 进行回溯拼接处理
            i = -2
            while len(flow.messages) >= -1 * i:
                if flow.messages[i].from_client:
                    break
                elif len(flow.messages[i].content) < 4090:
                    break
                else:
                    content = flow.messages[i].content + content
                i -= 1
    if not data_list:
        data_list = [content]
    user_name = get_user_name_by_ip_for_redis(client_ip)
    for content in data_list:
        if content == b'\x03\x01' or content == b'\x03\x81':
            in_black = True
        crypto, crypto_data = handle_tcp_crypto(content)
        data = {
            ...
            "conn_id": tcp_conns_dict.get(flow.client_conn.id + flow.server_conn.id, None),
            "crypto": crypto,
            "crypto_data": crypto_data,
            "user_name": user_name,
        }
        mes_id = db['tcp'].insert_one(data).inserted_id
        data['_id'] = str(mes_id)
        data['conn_id'] = str(data['conn_id'])
        data['content'] = base64.b64encode(data['content']).decode()
        data['type'] = 'tcp'
        if in_black:
            return
        if crypto:
            rc.lpush("mloger:tcp_data", json.dumps(data))
        else:
...
def tcp_start(flow):
    ...
    tcp_conns_dict[flow.client_conn.id + flow.server_conn.id] = conn_id
    data['_id'] = str(data['_id'])
    rc.publish("mloger:imws_websocket", json.dumps(data))
    asyncio.create_task(inject_tcp(flow))


async def inject_tcp(flow):
    # 注入功能
    while db['connections'].find_one({'type': 'tcp',
                                      'live': True,
                                      'client_id': flow.client_conn.id,
                                      'server_id': flow.server_conn.id, }):
        tcp_mes = db['inject_repeater'].find_one({'client_id': flow.client_conn.id,
                                                  'server_id': flow.server_conn.id, 'status': None, 'type': 'tcp'})
        if tcp_mes:
            if type(tcp_mes['content']) is str:
                tcp_mes['content'] = tcp_mes['content'].encode()
            to_client = False if tcp_mes['direction'] == 'send' else True
            if tcp_mes['concurrent_num'] > 0:
                for _ in range(tcp_mes['concurrent_num']):
                    ctx.master.commands.call("inject.tcp", flow, to_client, tcp_mes['content'])
            else:
                ctx.master.commands.call("inject.tcp", flow, to_client, tcp_mes['content'])
            db['inject_repeater'].update_one({'_id': tcp_mes['_id']}, {'$set': {'status': 'done'}})
...
def tcp_error(flow):
    ...
    data['_id'] = str(tcp_conns_dict.pop(flow.client_conn.id + flow.server_conn.id))
    data['live'] = False
    rc.publish("mloger:imws_websocket", json.dumps(data))
